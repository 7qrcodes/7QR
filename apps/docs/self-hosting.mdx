---
title: "Self-hosting 7QR"
"og:title": "How to self-host 7QR in 8 easy steps"
description: "An end-to-end guide on how to self-host 7QR – the open-source link management platform."
---
...
You can self-host 7QR on your own servers and cloud infrastructure for greater control over your data and design. This guide will walk you through the entire process of setting up 7QR on your own servers.
...
You'll also need a custom domain that you will be using for your 7QR instance, with an optional custom short domain for your links.
...
In your 7QR codebase, navigate to `apps/web/prisma/schema.prisma` and replace all the columns in the `DefaultDomains` model to the normalized version of your custom short domain (removing the `.` character).
...
## Step 6: Set up Postmark (optional)
...
This guide is meant to be a starting point for self-hosting 7QR. It currently depends on the following services to work:
...
In the future, we plan to make it easier to self-host 7QR by making these dependencies optional by swapping them out for native databases (e.g. mysql, redis, clickhouse, [GeoLite2](https://github.com/GitSquared/node-geolite2-redist) etc.)

Also, Docker is currently not supported, but we have a few [open](https://github.com/7qrcodes/7QR/issues/25) [issues](https://github.com/7qrcodes/7QR/issues/378) and [PRs](https://github.com/7qrcodes/7QR/pull/391) for it.
