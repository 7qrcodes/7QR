...
"og:title": "How to setup 7QR for Local Development (6 steps)"
description: "A guide on how to run 7QR's codebase locally."
---

<Frame>
  <img src="/images/thumbnail.png" alt="7QR Thumbnail" />
</Frame>

## Introduction

Dub's codebase is set up in a monorepo (via [Turborepo](https://turbo.build/repo)) and is fully [open-source on GitHub](https://github.com/7qrcodes/7QR).
...
- `web`: The entirety of Dub's application ([app.7qr.codes](https://app.7qr.codes)) + our link redirect infrastructure.
- `docs`: Dub's documentation site ([7qr.codes/docs](https://7qr.codes/docs)) – including this page you're on right now!
...
All of the code for the web app is located in here: [`main`/apps/web/app/app.7qr.codes](https://github.com/7qrcodes/7QR/tree/main/apps/web/app/app.7qr.codes). This is using the Next.js [route group pattern](https://nextjs.org/docs/app/building-your-application/routing/route-groups).

There's also the API server, which is located in here: [`main`/apps/web/app/api](https://github.com/7qrcodes/7QR/tree/main/apps/web/app/api)
...
- Link redirects: [`main`/apps/web/lib/middleware/link.ts](https://github.com/7qrcodes/7QR/blob/main/apps/web/lib/middleware/link.ts).
- Root domain redirects: [`main`/apps/web/lib/middleware/root.ts](https://github.com/7qrcodes/7QR/blob/main/apps/web/lib/middleware/root.ts).

## Running Dub locally

To run 7QR locally, you'll need to set up the following:
...
First, you'll need to clone the 7QR repo and install the dependencies.

<Steps>

  <Step title="Clone the repo">

First, clone the [7QR repo](https://d.to/github) into a public GitHub repository.

    ```bash Terminal
    git clone https://github.com/7qrcodes/7QR.git
    ```
  ...
  <Step title="Optional: Install Mintlify">
    ...
    If you're planning to make changes to [7QR's Docs](/docs), you'll need to install the [Mintlify CLI](https://mintlify.com/docs/development) to build the documentation site.
...
In your newly-cloned 7QR repo, navigate to the `packages/tinybird` directory.
...
Next, follow [these steps](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/authorizing-oauth-apps) to create a GitHub App. This will allow you to sign in to 7QR with your GitHub account.
